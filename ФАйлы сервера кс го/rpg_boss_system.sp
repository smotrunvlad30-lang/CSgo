...
public void OnClientPutInServer(int client) {
    ...
    // Перехват бота для босса
    if (g_bBossActive && g_iBossClient == -1 && IsFakeClient(client)) {
        char name[32]; GetClientName(client, name, sizeof(name));
        if (StrEqual(name, g_sBossName)) {
...
public Action Timer_RenameBot(Handle timer) {
    for (int i = 1; i <= MaxClients; i++) {
        if (IsClientInGame(i) && IsFakeClient(i)) {
            ...
            if (!StrEqual(name, g_sBossName) && g_iBossClient == -1) {
                SetClientName(i, g_sBossName);
                g_iBossClient = i;
                SetupBoss(i);
                return Plugin_Stop;
            } else if (StrEqual(name, g_sBossName)) {
                g_iBossClient = i;
                SetupBoss(i);
                return Plugin_Stop;
            }
...
public Action Timer_UpdateHud(Handle timer) {
    ...
    if (timeLeft <= 0) { EndBossFight(); ServerCommand("bot_kick %s", g_sBossName); return Plugin_Stop; }
